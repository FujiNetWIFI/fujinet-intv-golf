...
; Demo script for PGA Golf.  Column dec0/kp0 = seat 0 (dec0 -> $011F when
; it is seat 0's turn); dec1/kp1 = seat 1.  Seats 2/3 share these same two
; columns by parity (seat AND 1) -- the turn arbiter (ARB_COMPUTE,
; src/hook.asm) only ever replays the ONE seat whose turn it actually is
; ($016B, the game's own "whose turn" cell), so shared columns are
; provably ignored off-turn (Bowling's precedent, PORTING.md §7.23).
;
; The player-count prompt is answered by ARB_INJECT, not this script --
; confirmed live (M1): a headless run with NO script content driving that
; prompt still landed $0161 (player count) at NET_COUNT and progressed
; into real play (GAME_TBL == GF_HTBL_LIVE = $51D3), with the hole number
; ($016C) advancing 0 -> 1 and a real terrain/lie code set at $0183.  So
; this script's job is purely to exercise the DISC path once play has
; started -- the exact swing/aim mechanic ($525E's own $011F/$0120 poll,
; separate from the $035D handler table) was NOT reverse-engineered this
; session; varied sustained disc holds were confirmed (M1) to be enough
; to drive real hole progression, which is the strongest destination-
; phase signal available without a live-scan measurement session (M2
; TODO, PORTING.md §5.5 -- confirm exact event codes/timing against a
; live $011F dump before trusting this beyond "does something happen").
;
; Coverage this script reaches, in order:
;   1. settle briefly (injector answers the count prompt automatically,
;      regardless of what this script is doing at the same time)
;   2. varied disc holds, both columns, long enough to cover several
;      strokes' worth of ticks at the measured 20 Hz tick rate (M2)
...
SCRIPT_TBL:
        ...
        ; Varied sustained disc holds, both seat-parity columns, cycling
        ; through several directions with generous dwell time -- confirmed
        ; live (M1) to be enough to drive real hole progression even
        ; without knowing the exact swing/aim mechanic.  Two full passes
        ; to cover more than one stroke/hole.
        DECLE   15, $02, $02, 0, 0
        DECLE   5,  $40, $40, 0, 0
        DECLE   15, $0C, $0C, 0, 0
        DECLE   5,  $40, $40, 0, 0
        ...
        DECLE   5,  $40, $40, 0, 0
        DECLE   15, $08, $08, 0, 0
        DECLE   20, $40, $40, 0, 0      ; let physics/turn transitions settle

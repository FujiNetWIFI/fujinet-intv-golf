...
TRACE_RANGES:
        ...
        ; Virtualized timer state.  GF_ARM1/2/3 + GF_CNT2 carry real state
        ; (7 cart timer-API sites) -- the ranges keep the family's exact
        ; spans, covering the always-zero RS_SPARE cells too, so that these
        ; bounds, LS_CKSUM's and RS_TAILTBL's stay in agreement without a
        ; §7.16 sweep.  Zeros checksum as zeros.
        DECLE   GF_ARM1, GF_ARM1+1      ; $8102
        DECLE   GF_ARM3, GF_ARM3+1      ; $810F
        DECLE   GF_CNT2, RS_SPARE1+1    ; $8196-$819B (CNT2 + 2 spares,
                                        ;  ARM2 + 2 spares)
        DECLE   ARB_INJ, ARB_INJ+1      ; $819F -- the player-count prompt

...
@@ls_go0:
        ...
        ADDR    R1,     R2              ; R2 = T+d
    IF NET_FUZZ <> 0
        ; Rig builds: the demo script first (the player-count prompt digit +
        ; ENTER is answered by ARB_INJECT, not this script -- the script
        ; drives the disc: tee shots, putts), then pseudo-random input
        ; seeded per console from boot entropy.  The script has two columns
        ; and seats 2/3 share them (the turn arbiter only replays the seat
        ; whose turn it is, so shared columns are provably ignored off-turn
        ; -- Bowling's precedent, PORTING.md §7.23).  Masked to $3F below
        ; (disc-space only): keypad coverage belongs to the script, where
        ; it is reproducible (PORTING.md §5.5).
...
@@ls_run:
        ...
        EIS
@@ls_nf:
        ; Golf POLLS: a single walking-pointer read at $525F drains the
        ; shadow pair ($011F then $0120).  UPDATE_SHADOW seeds it from the
        ; live cells; SHADOW_FROM_RINGS below overwrites it with the
        ; ARBITER SEAT's ring value, which is the call whose absence only a
        ; real two-console rig can detect (PORTING.md §7.20).
        ...
        MVO     R1,     $35D
@@ls_no_tbl:
        ; Turn-arbitered virtual dispatch BEFORE the tick (replay-before,
        ; the EXEC-loop default confirmed by the virt==hook gate at M5):
        ; ARB_COMPUTE picks the ONE controlling seat from sim state --
        ; identical on every console by lockstep -- or runs the
        ; player-count injector.  Only that seat's ring events replay.
        JSR     R5,     ARB_COMPUTE
        JSR     R5,     SHADOW_FROM_RINGS
        MVI     ARB_SEAT, R0
        CMPI    #$FF,   R0
        BEQ     @@ls_no_disp            ; injector owned this tick
        MVO     R0,     VD_SIDE
        JSR     R5,     LS_VDISPATCH
@@ls_no_disp:
        JSR     R5,     GF_GAME_TICK    ; all three entries, arm-gated inside
        JSR     R5,     LS_TBL_ADOPT
...
@@ls_nocrc:
        ...
        MVO     R0,     TICK_HI
@@ls_pend:
        ; current golfer's name row, repainted every 16 ticks (HUD cadence).
...
; ---------------------------------------------------------------------------
; NAME_DRAW -- paint the CURRENT GOLFER's name in their ball colour:
; "n NAME".  Row 11 normally; row 0 in HUD builds (the HUD owns row 11
; there).  ARB_SEAT is already the right value for display: ARB_COMPUTE
; (src/hook.asm) sets it to GF_CUR every tick, the same cell the turn
; arbiter itself uses to pick who plays -- no separate derivation needed.
; Injector ticks ($FF) keep the previous seat's row -- no flicker while
; the player-count prompt is up.
; ---------------------------------------------------------------------------
    IF NET_HUD <> 0
NAMEROW         EQU     0
    ELSE
...
; (GF_ARM1/2/3 + GF_CNT2 -- 7 cart arm/stop sites make these hot sim
; state) + ARB_INJ, the player-count prompt injector counter.  Must match
...
@@lk_1: SLLC    R0,     1
        ...
        SLLC    R0,     1
        ADCR    R0
        ; Virtualized timer tail.  GF_ARM1/2/3 and GF_CNT2 carry real state
        ; (entry 2's interval is 3, the only one needing a countdown); the
        ; spans stay exactly the family's so this tail, debug.asm's
        ; TRACE_RANGES and resync.asm's RS_TAILTBL agree without a §7.16
        ; bounds sweep.  The RS_SPARE cells are always zero and contribute
        ; nothing.
        ADD     GF_ARM1, R0
        SLLC    R0,     1
        ADCR    R0
        ADD     GF_ARM3, R0
        MVII    #GF_CNT2, R4            ; $8196-$819B: CNT2 + 2 spares,
                                        ;  ARM2 + 2 spares
@@lk_2: SLLC    R0,     1
        ADCR    R0
        ADD@    R4,     R0

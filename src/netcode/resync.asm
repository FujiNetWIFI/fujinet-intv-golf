...
;
; NOTE (Golf, M0): this file is copied from Sea Battle's tree per
; PORTING.md §4's "copy unchanged" list, with exactly ONE line added to
; RS_REBASE below (a JSR to GF_REBASE_HOOK, defined in src/hook.asm).
; RS_CLAMP_ISR immediately above it is kept as a structural no-op (Golf has
; no cart ISR at all -- see src/exec_equ.asm's SC_ISR_SAVE/SC_ISR_BODY
; comment) rather than removed, so this file's only cart-specific content
; stays the one added call, exactly like Sea Battle's own precedent.
;
...
;
; State image (769 bytes, positions 0..768):
;   0..146    $015D-$01EF game scratch (bytes; the whole Golf game state --
;             player count, stroke counts, current player, hole number,
;             play order, terrain/lie codes)
;   147..626  $0200-$02EF BACKTAB (240 words, LE byte pairs -- the course
;             card the physics engine reads back, and the scoreboard,
;             travel for free -- see spikes/NOTES.md M3 for the audit of
;             whether this read-back-as-input cell class ever diverges)
;   627..754  $031D-$035C object table (64 words, LE byte pairs; the ball
;             and golfer sprites.  Golf keeps no cart globals above the
;             stack base -- M0 census)
;   755..768  RNG_LO/HI, GF_ARM1, GF_ARM3, GF_CNT2 + two spares (the block
;             $8196-$819B), GF_ARM2 + two spares, ARB_INJ, GF_PASSLEN
;             ($0103 -- GAME-WRITTEN on this cart, $5085 writes 3),
;             GAME_TBL_LO/HI.  The spares are always zero: keeping the
;             family's exact 14-entry tail is what lets every IMG_* bound
;             below stand unswept (PORTING.md §7.16).
...
@@rb_hd:
    ENDI
        ...
        JSR     R5,     RS_CLAMP_ISR
        JSR     R5,     GF_REBASE_HOOK  ; cart-specific (src/hook.asm):
                                        ;  clamps GF_CUR/GF_ORDER_IDX/
                                        ;  GF_HOLE (§7.27 audit)
        MVI     RS_R_LO, R0
        MVO     R0,     TICK_LO
        MVI     RS_R_HI, R0
...
;
; $0160/$0161 is where L_55B6 stashes $0100/$0101 before installing the
; cart's own ISR body, and the body copies it back unconditionally at
; $561D.  It sits inside $015D-$01EF, so it rides in image section 1 whether
; we want it to or not.  In a healthy machine it is always $1126 and this
; routine is a no-op -- but a single corrupted wire byte landing there would
; install an ARBITRARY ADDRESS as the frame interrupt handler on the next
; tick and brick the console.  None of the existing image guards can catch
; it: they bounds-check the POSITION, and this position is legal.
...
; ---------------------------------------------------------------------------
; 14 entries, deliberately the same count and the same addresses the family
; has used since Bowling: GF_ARM1/2/3 (armed flags) and GF_CNT2 (the one
; countdown, entry 2's interval is 3) carry real state; the RS_SPARE* cells
; are always zero.  Keeping the layout fixed means IMG_S3/IMG_TOTAL and
; every bounds guard stay exactly as proven (PORTING.md §7.16).
RS_TAILTBL:
        DECLE   RNG_LO, RNG_HI
        DECLE   GF_ARM1, GF_ARM3
        DECLE   GF_CNT2, RS_SPARE3, RS_SPARE4    ; $8196-$8198
        DECLE   GF_ARM2, RS_SPARE2, RS_SPARE1    ; $8199-$819B
        DECLE   ARB_INJ, GF_PASSLEN

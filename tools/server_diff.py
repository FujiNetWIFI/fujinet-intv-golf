...
Usage:
    tools/server_diff.py                    # all scenarios, 4 seats (Golf)
...
def main():
    ...
    if PROTO == 2 and args.seats != 4:
        # This repo's Python is pinned at MAX_SEATS = 4 (Golf seats a
        # foursome), so there is nothing to diff against at any other
        # width -- matching the s2_rooms scenario's own `seats >= 3`
        # branch (redirect into a forming room) rather than the 2-seat
        # "refused, room full" branch.
        sys.exit("--seats is only diffable at 4 (the Python here is pinned "
                 "at MAX_SEATS=4)")

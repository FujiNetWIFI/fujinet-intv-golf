#!/usr/bin/env python3
"""Intellivision netplay matchmaking + relay server (PGA Golf instance).
...
GOLF IS 2-4 PLAYERS, turn-arbitered on the console side (one golfer's
input reaches the sim per tick, computed from sim state).  MAXPLAYERS is 4
here; the seat plumbing is unchanged from the Bowling/Sea Battle lineage.
...
PROTO_VER = 2
DEFAULT_DELAY = 3
# Room capacity.  Golf seats up to a foursome.
MAX_SEATS = 4
...
class LobbyPublisher:
    ...
    def __init__(self, base_url, serverurl, client_url, appkey, region="us",
                 game_name="PGA Golf", server_name="Golf Netplay"):
...
def main():
    ...
    ap.add_argument("--game-name", default="PGA Golf",
                    help="game name for the FujiNet Lobby registration")
    ap.add_argument("--server-name", default="Golf Netplay",
    ...
    ap.add_argument("--lobby-serverurl",
                    default="TCP://fujinet.online:9111/",
    ...
    ap.add_argument("--lobby-appkey", type=int, default=19,
                    help="FujiNet-registry appkey id for this game "
                         "(PGA Golf = 19; the Lobby rejects 0)")
    args = ap.parse_args()
    logging.basicConfig(

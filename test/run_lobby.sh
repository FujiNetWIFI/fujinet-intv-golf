...
FN1=$!
( relay_server --port 9111 ) > "$RIG/lb_server.log" 2>&1 &
SRV=$!
...
MENU_RD=$(awk '/CMP  *MENU_PREV, R0/ {print $1; exit}' "$BUILD/golf_net.lst")
[ -n "$MENU_RD" ] || { echo "run_lobby.sh: cannot find MENU_PREV compare"; exit 1; }
echo "menu read site: \$$MENU_RD"
HOLD_ADDR=$(awk '/ SES_HOLD$/ { sub(/^0+/, "", $1); print $1 }' "$BUILD/golf_net.sym")
...
SDL_VIDEODRIVER=dummy SDL_AUDIODRIVER=dummy \
    ...
    "$BUILD/golf_net.bin" > "$RIG/lb1.out" 2>&1 || true
...
SDL_VIDEODRIVER=dummy SDL_AUDIODRIVER=dummy \
    ...
    --fujinet=localhost:19851 -e rom/exec.bin -g rom/grom.bin \
    "$BUILD/golf_net.bin" > "$RIG/lb2.out" 2>&1 &
C2=$!
sleep 8

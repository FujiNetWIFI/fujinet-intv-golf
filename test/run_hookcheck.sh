#!/bin/sh
# NOT YET ADAPTED FOR GOLF -- inherited verbatim from Sea Battle/Soccer and
# still entirely Soccer-shaped (kickoff phase $0179, two-team simultaneous
# play, 4 countdown entries, an ISR dance Golf doesn't have).  It will FAIL
# or hang against a golf_hook.bin build; do not trust its output.
# NOT part of the canonical gate ladder (PORTING.md §8 doesn't list a
# hookcheck step) -- Golf's own §7.31 live-boot-dump check (spikes/
# NOTES.md M1) already covers what this script exists to prove (the
# relocated table + MASTER_TICK dispatch actually run), and does so
# without needing the exact swing/aim mechanic this script would require
# rewriting for (see spikes/NOTES.md's open items).  Rewriting this for
# real play (not just boot-dump) is future work once that mechanic is
# reverse-engineered.
#
# Sea Battle's original header, kept for reference on what a hookcheck
# ONCE looked like when the underlying cart matches:

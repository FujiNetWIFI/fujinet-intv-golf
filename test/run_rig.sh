...
#   1x intv_relay_server (:9111, --auto-go N)
...
if ! grep -q '127\.0\.0\.1' "$BUILD/srv_endpoint.asm" 2>/dev/null; then
    ...
    echo "  make SRV_HOST=127.0.0.1 build/golf_net1.bin ..."
    exit 1
fi
...
while [ "$i" -le "$PLAYERS" ]; do
    ...
    SDL_VIDEODRIVER=dummy SDL_AUDIODRIVER=dummy \
        ...
        "$BUILD/golf_net$i.bin" > "$RIG/c$i.out" 2>&1 &
    CONS="$CONS $!"
    sleep 2
    i=$((i+1))
done
...
for n in range(1, players + 1):
    ...
    print(f"console {n}: name={name!r} seat={seat} count={count} "
          ...
          f"pcount={pcount} hole={hole} roster={roster}")
    seats.add(seat)
    ...
    # produce a keypad digit or ENTER on its own -- but Golf's player-count
    # prompt is answered independently by ARB_INJECT (sim-state-driven, not
    # input-driven, src/hook.asm), confirmed live (M1) to get a fuzz-only
    # run all the way into real play.  So, unlike a cart with no such
    # injector, this rig CAN and must assert real progress: the player
    # count landed in range, and GAME_TBL moved off the prompt table.
    ...
    if gtbl == GF_HTBL_PROMPT:
        print(f"console {n}: PARKED ON THE PLAYER-COUNT PROMPT "
              f"(GAME_TBL=$51DD) -- every dump agrees trivially")
        ok = False

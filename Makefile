# Networked Intellivision PGA Golf (Mattel 1979) over FujiNet.
# 2-4 player lockstep netplay, turn-arbitered.
...
# No build may ever map $7000: the EXEC boot EXECUTES whatever is there
# (the ECS expansion hook).  Caught live: net builds booted or crashed
# purely on which instruction their $7000 word happened to encode.
check-7000:
	@bad=0; for f in $(BUILD)/*.cfg; do \
	    grep -q '= \$$7000' $$f && { echo "FATAL: $$f maps \$$7000 (EXEC boot executes it)"; bad=1; }; \
	done; exit $$bad
...
# PLAYERS=2..4 (default 2) scales it, Bowling-style: make rig PLAYERS=4
PLAYERS  ?= 2
...
rig: $(RELAY_DEP)
	$(MAKE) SRV_HOST=127.0.0.1 SRV_PORT=9111 $(RIG_BINS)
	$(MAKE) check-7000
...
lobby: $(RELAY_DEP)
	$(MAKE) SRV_HOST=127.0.0.1 SRV_PORT=9111 $(BUILD)/$(GAME)_net.bin
